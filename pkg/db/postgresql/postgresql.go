...
func (p Postgresql) Save(entry *url.Entry) (*url.Entry, *errs.AppError) {
	...
	if err != nil {
		p.log.Errorf("Could not save Entry to store: %v", err)
	...
	if err != nil {
		if err == sql.ErrNoRows {
			p.log.Errorf("Could not find saved Entry: %v", err)
			return nil, errs.NewUnexpectedError("unexpected database error")
		}
		p.log.Errorf("Could not scan Entry: %v", err)
		return nil, errs.NewUnexpectedError("unexpected database error")
	}
...
func (p Postgresql) Find(code string) (*url.Entry, *errs.AppError) {
	sqlSelect := "SELECT * from urls where code = $1"
